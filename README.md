...
- 按模块分类管理常用命令
- 支持新增/编辑/删除模块和命令
- 支持 **GitBook 格式**文档库（Markdown 章节文档，支持 KaTeX 数学公式和 Mermaid 流程图）
- 支持导入/导出 JSON 格式的文档库
- 团队协作知识库
...
| 功能 | 说明 |
|------|------|
| 文本统计 | 字符数、字节数(UTF-8)、占据大小、行数、单词数、中文字符、英文字母、数字字符 |
| MD5 哈希 | 实时计算文本 MD5 值 |
| 16进制转换 | 文本↔16进制双向转换，支持空格/逗号/无分隔符，可选 0x 前缀（大写） |

### 🎨 绘图工具

| 工具 | 说明 |
|------|------|
| **Draw.io 流程图** | 在线模式，支持导入/导出 .drawio、.xml 文件，保存为 PNG 图片 |
| **Excalidraw 手绘流程图** | 离线模式，支持导入/导出 .excalidraw JSON 文件，保存为 PNG 图片 |
| **Drawnix 思维导图** | 离线模式，基于 Plait 框架，支持多种图表类型，自动保存 |
...
9. **绘图工具** — Draw.io（在线）/ Excalidraw（离线 npm）/ Drawnix（离线 iframe）三大绘图工具
10. **Markdown 编辑器** — 支持 KaTeX 数学公式和 Mermaid 流程图，三种编辑模式
11. **GitBook 格式** — 命令库支持 Markdown 章节文档格式
12. **文本工具增强** — MD5 哈希计算、16进制转换（大写 0x 前缀）
13. **插件审核机制** — 用户提交插件需管理员审核，支持审核日志记录
